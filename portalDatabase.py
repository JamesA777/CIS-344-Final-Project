...
class Database():
    ...
    def addPatient(self, patient_name, age, admission_date, discharge_date):
        ...
        if self.connection.is_connected():
            ...
            return
    def addDoctor(self, doc_id, d_name, email, phone, specialization):
        ''' Method to insert a new doctor into the doctor table '''
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "INSERT INTO doctors (doc_id, d_name, email, phone, specialization) VALUES (%s, %s, %s, %s, %s)"
            self.cursor.execute(query, (doc_id, d_name, email, phone, specialization))
            self.connection.commit()
            return
    ...
    def dischargePatient(self, id):
        ...
        if self.connection.is_connected():
             self.cursor = self.connection.cursor()
             query = "UPDATE patients SET discharge_date = current_date WHERE patient_id = id"
